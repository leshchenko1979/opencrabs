...
def _run_fastmcp(
    ...
    if use_json:
        cmd.append("--json")
    return subprocess.run(cmd, capture_output=True, text=True, timeout=DEFAULT_TIMEOUT + 10)


def _lean_stdout(
    proc: subprocess.CompletedProcess[str], tool: str, args_json: str, bearer: str, server_url: str
) -> str:
    ...
    logger.info("fallback --json for tool=%s", tool)
...
def main() -> None:
    ...
    try:
        ...
        logger.error("invalid json args tool=%s: %s", tool, exc)
        print(json.dumps({"error": f"invalid json args: {exc}"}))
        sys.exit(1)

    bearer = _read_bearer()
    if not bearer:
        logger.error("missing bearer tool=%s", tool)
        print(json.dumps({"error": "TG_MCP_BEARER unset and no [mcp] bearer in config"}))
        sys.exit(1)

    server_url = _read_server_url()
    args_preview = args_json if len(args_json) <= 500 else args_json[:500] + "…"
    logger.info("call start tool=%s url=%s args=%s", tool, server_url, args_preview)
    ...
    try:
        ...
        if len(err_msg) > 800:
            err_msg = err_msg[:800] + "…"
        logger.error("call failed tool=%s elapsed_ms=%d error=%s", tool, elapsed_ms, err_msg)
