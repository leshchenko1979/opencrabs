...
2. **Decide action**: After reading:
   ...
   - If the file already covers the topic AND the feedback shows a FRESH repeat violation (new incident since the rule was written) → use action='update' to reinforce: append the new date/incident as evidence. Do NOT bump inline counters — the SQLite DB is the source of truth for event counts.
...
If the rule already covers the pattern but needs reinforcement: append a new evidence entry
(date/session/incident) to the existing rule via action='update'. Do NOT bump inline counters
in brain files — they are decorative and stale, not authoritative. The SQLite feedback ledger
at `~/.opencrabs/feedback.db` is the single source of truth for event counts. Use
`feedback_analyze` to read current counts. Brain file counters are not read by the runtime.
