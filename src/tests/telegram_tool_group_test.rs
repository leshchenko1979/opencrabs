...
fn single_tool_without_context_omits_trailing_space() {
    ...
    assert!(out.contains(
        "<blockquote expandable><b>⛏ web_search</b>\n\n<b>⚙️ web_search</b></blockquote>"
    ));
...
fn multiple_tools_render_expandable_blockquote() {
    ...
    assert!(out.starts_with("⚙ • 0:00 ⏱\n<blockquote expandable><b>⛏ grep pattern</b>\n\n"));
...
fn running_tool_fallback_does_not_double_the_gear() {
    ...
    assert!(out.contains("<blockquote expandable><b>⛏ bash gh pr list</b>"));
}
...
fn settled_outcome_renders_block_header_over_lone_tool() {
    ...
    assert_eq!(
        out,
        "❌ • 0:00 ⏱\n<blockquote expandable><b>⛏ bash cargo test</b>\n\n<b>✅ bash</b> <code>cargo test</code></blockquote>"
    );
}
...
fn rich_live_status_in_header() {
    ...
    assert!(out.starts_with("⛏ **grep pattern** • _2 tool calls_ • _10s_\n\n"));
}
...
fn details_single_tool_renders_details_block() {
    ...
    assert_eq!(
        out,
        "<p>⚙ • 0:00 ⏱</p><details><summary><sub>⛏ bash git status</sub></summary>\
...
fn details_multiple_tools_wrap_in_collapsed_details() {
    ...
    assert!(
        out.starts_with("<p>⚙ • 0:00 ⏱</p><details><summary><sub>⛏ read file.rs</sub></summary>")
    );
...
fn test_tool_class_icons() {
    ...
    assert_eq!(
        ToolClass::from_tool_name("suggest_options").class_icon(),
        "💡"
    );
    ...
    assert_eq!(tool_entry_icon("suggest_options", Some(true)), "💡");
    ...
    let (icon, target) = crate::channels::telegram::flow::clean_tool_entry(
        "suggest_options",
        Some(true),
        "2 options",
    );
    assert_eq!(icon, "💡");
    assert_eq!(target, "2 options");
