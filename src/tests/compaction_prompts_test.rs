...
use crate::brain::agent::service::compaction_prompts::{
    CompactionKind, PlanRecovery, append_skill_stamp, append_tool_stamp, build_continuation,
};
...
const TOOL_STAMP_HEADER: &str = "LAZY TOOLS LOADED PRE-COMPACTION:";
const TOOL_STAMP_CAVEAT: &str = "loaded at compaction time";

#[test]
fn tool_stamp_lists_active_lazy_tools_with_recency_caveat() {
    let body = build_continuation(CompactionKind::Regular, false, true, PlanRecovery::Active);
    let stamped = append_tool_stamp(
        body,
        &["telegram_send".to_string(), "browser_navigate".to_string()],
    );
    assert!(
        stamped.contains(TOOL_STAMP_HEADER),
        "stamp must carry the inventory header: {stamped}"
    );
    assert!(
        stamped.contains("telegram_send") && stamped.contains("browser_navigate"),
        "stamp must list every active lazy tool: {stamped}"
    );
    assert!(
        stamped.contains(TOOL_STAMP_CAVEAT),
        "stamp must carry the recency caveat (LRU cap may have evicted older entries): {stamped}"
    );
}

#[test]
fn tool_stamp_is_silent_when_no_lazy_tools_active() {
    for kind in [
        CompactionKind::Regular,
        CompactionKind::MidLoop,
        CompactionKind::Emergency,
        ...
        for silent in [false, true] {
            let body = build_continuation(kind, silent, true, PlanRecovery::NoPlan);
            let stamped = append_tool_stamp(body, &[]);
            assert!(
                !stamped.contains(TOOL_STAMP_HEADER),
                "{kind:?} silent={silent}: no lazy tools → no stamp, zero marginal tokens: {stamped}"
            );
        }
    }
}
...
#[test]
fn tool_stamp_rides_all_kinds_and_variants() {
    let tools = vec!["session_search".to_string()];
    for kind in [
        CompactionKind::Regular,
        CompactionKind::MidLoop,
        CompactionKind::Emergency,
        ...
        for silent in [false, true] {
            let body = build_continuation(kind, silent, true, PlanRecovery::Active);
            let stamped = append_tool_stamp(body, &tools);
            assert!(
                stamped.contains(TOOL_STAMP_HEADER) && stamped.contains(TOOL_STAMP_CAVEAT),
                "{kind:?} silent={silent}: stamp must ride every kind/variant: {stamped}"
            );
        }
    }
...
#[test]
fn tool_stamp_sorts_names_for_determinism() {
    let body = build_continuation(CompactionKind::Regular, false, true, PlanRecovery::NoPlan);
    let stamped = append_tool_stamp(
        body,
        &[
    ...
    let header_pos = stamped.find(TOOL_STAMP_HEADER).expect("stamp present");
    let list = &stamped[header_pos..];
    let alpha = list.find("alpha_tool").expect("alpha_tool present");
    let mid = list.find("mid_tool").expect("mid_tool present");
    let zeta = list.find("zeta_tool").expect("zeta_tool present");
    assert!(
        alpha < mid && mid < zeta,
        "stamp list must be sorted for deterministic rendering: {list}"
    );
...
#[test]
fn tool_stamp_rides_after_skill_stamp_in_continuation_prompt() {
    // The lazy-tool stamp is the FINAL block: it wraps the skill-stamped
    // text, so the skill header must appear strictly before the tool one.
    let body = build_continuation(CompactionKind::Regular, false, true, PlanRecovery::NoPlan);
    let both = append_tool_stamp(
        append_skill_stamp(body, &["grafana".to_string()]),
        &["telegram_send".to_string()],
    );
    let skill_pos = both.find(STAMP_HEADER).expect("skill stamp present");
    let tool_pos = both.find(TOOL_STAMP_HEADER).expect("tool stamp present");
    assert!(
        skill_pos < tool_pos,
        "tool stamp must ride after the skill stamp (final block): {both}"
    );
}

#[test]
fn tool_stamp_names_only_no_schemas() {
    let body = build_continuation(CompactionKind::Regular, false, true, PlanRecovery::NoPlan);
    let stamped = append_tool_stamp(body, &["telegram_send".to_string()]);
    // Names only: no JSON braces, no schema keywords.
    assert!(
        !stamped.contains('{') && !stamped.contains("parameters"),
        "tool stamp must carry names only, no schemas: {stamped}"
    );
}
