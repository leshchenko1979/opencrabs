...
fn flow_telemetry_html_dom_placement() {
    ...
    assert!(
        html.contains("⚙️ 2 • ⏱ 0:42 • ⏏️ 1 • 🤖 2 • ✉️ 3"),
    ...
    let telem_pos = html.find("⚙️ 2 • ⏱ 0:42").expect("telemetry bar present");
    let quote_pos = html
...
fn flow_telemetry_details_dom_placement() {
    ...
    assert!(
        details_html.contains("<p>⚙️ 2 • ⏱ 0:42 • ⏏️ 2 • ✉️ 1</p>"),
...
fn flow_telemetry_settled_outcome_renders() {
    ...
    let html = render_flow(&s);
    assert!(
        html.contains("⚙️ 2 • ⏱ 0:42 • ✉️ 4"),
        "Settled outcome includes telemetry: {html}"
    );

    let details_html = render_flow_details_state(&s);
    assert!(
        details_html.contains("<p>⚙️ 2 • ⏱ 0:42 • ✉️ 4</p>"),
